...
dialog.querySelector("span").addEventListener("click", closepopup);
...
for(let i = 0; i < 6; i++){
  ...
  header.innerText = picArray[i]["title"]
  article.appendChild(header);

  let figure = document.createElement("figure");

  let imageElement = document.createElement("img");
  imageElement.setAttribute("src", picArray[i]["image"]["medium"]);
  imageElement.setAttribute("alt", picArray[i]["title"]);
  ...
  article.setAttribute("onclick", `popup(${i})`);
  
  section.appendChild(article);
}

function popup(num){
  dialog.querySelector("img").setAttribute("src", picArray[num]["image"]["large"]);
  dialog.querySelector("img").setAttribute("alt", picArray[num]["title"]);
  dialog.showModal();
}
